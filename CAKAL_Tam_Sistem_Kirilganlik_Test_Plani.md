...
**Birim testi durumu:** 69 dosya / 895 test, tamamı yeşil (`npx vitest run`, 12 Ağustos 2026 20:24). Son ekleme: `supabase-realtime-transport.test.mjs` (12 test) — bildirilmemiş `ws` bağımlılığı ve sessiz yutulan transport hatası.  
...
`49d1812` üzerinde 69 dosya / 895 test yeşil. Aşağıdaki bölümlerin deterministik kısmı zaten kod içinde ölçülüyor; yapılacak iş **yeni test yazmak değil, plan ID’lerini mevcut testlerle eşlemek**.
...
1. **ID eşlemesi:** §2.1’deki tabloyu kullanarak 895 testin hangi plan ID’sini karşıladığını dosya bazında işaretle. Tek başına planın yarısını “zaten PASS” diye kapatır; asıl çıktısı hangi ID’nin **karşılanmadığı**dır.
