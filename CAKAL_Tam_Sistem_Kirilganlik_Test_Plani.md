...
**Birim testi durumu:** 71 dosya / 936 test, tamamı yeşil (`npx vitest run`, 12 Ağustos 2026 20:24). Son ekleme: `supabase-realtime-transport.test.mjs` (12 test) — bildirilmemiş `ws` bağımlılığı ve sessiz yutulan transport hatası.  
...
`49d1812` üzerinde 71 dosya / 936 test yeşil. Aşağıdaki bölümlerin deterministik kısmı zaten kod içinde ölçülüyor; yapılacak iş **yeni test yazmak değil, plan ID’lerini mevcut testlerle eşlemek**.
...
|---|---|---|
| ~~TEK-09, TEK-10, TEK-11~~ | **KAPANDI (12 Ağustos 2026).** Kanıt olayı artık ölçüm DEĞERLERİNİ taşıyor; kapı rakamın ölçümle aynı büyüklük mertebesinde olmasını arıyor ve sözleşme `COMPLETE` değilse hiç rakam geçirmiyor. `tests/level-derivation.test.mjs` (16 test) | TEK-10 artık PASS. Canlı turda yalnız **yanlış pozitif** aranmalı: meşru bir stop/hedef haksız yere bloklanıyor mu |
...
1. **ID eşlemesi:** §2.1’deki tabloyu kullanarak 936 testin hangi plan ID’sini karşıladığını dosya bazında işaretle. Tek başına planın yarısını “zaten PASS” diye kapatır; asıl çıktısı hangi ID’nin **karşılanmadığı**dır.
