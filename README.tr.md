...
> 🇬🇧 [English README](README.md) · Bu sayfa Türkçe tam sürümdür.
...
npm test               # 1058 birim testi (78 dosya)
...
- [ ] **Seviye türetim KANITI (bir sonraki iş):** kapı artık rakamın ölçümle aynı mertebede olmasını arıyor, ama seviyenin NASIL türetildiğini sormuyor. Zorunlu olması gerekenler: salınım dipleri/tepeleri, destek/direnç, ATR tamponu, giriş yöntemi (kırılım/dönüş), hedef yöntemi, risk/getiri, vade, formül ve dayanak kanıt kimliği. Canlı MEYSU turunda seviyeler yalnız `MARKET_SESSION_STATUS` eksik diye engellendi — oysa seans durumu bir zaman etiketidir, türetim kanıtı değildir. Seans verisi gelince kapı açılabilir ve model yine seviye uydurabilir
- [ ] **Nakit köprüsü mutabakatı:** `operating + investing + financing + fx === balanceDelta` kontrolü ve `reconciled` bayrağı. Kapanmayan köprüde "işletmeden geldi" / "finansmanla makyaj yok" hükmü kurulamamalı
- [ ] **Tek composer akışı:** onarım cevaptan ÖNCE bitmeli. Bugün ilk kompozisyon (~72s) tamamen israf; kapsam brifingi doğru çalışıyor ama yanlış yerde
- [ ] **Onarım aracını zorla:** sözleşme "eksik kanıtı toplayacak araç: `get_bist_board`" diyor, model başka araç çağırıyor, kimse itiraz etmiyor
- [ ] **Kanıt sınıfı ayrıştırması:** `YOUTUBE_SEARCH_METADATA` ≠ `SOCIAL_SENTIMENT_EVIDENCE` ≠ `EXPERT_RESEARCH_EVIDENCE`. Bugün "2 video bulundu" `RESEARCH_EVIDENCE` COMPLETE sayılıyor
- [ ] **Sunum kuralları:** rapor dönemi / karşılaştırma dönemi / verinin alınma zamanı ayrı yazılmalı; tablolarda birim; İş Yatırım enflasyon düzeltmesi beyan edilmeli; `NOT_EXTENDED` yalnız teknik uzamadır, "ucuz" demek değildir
- [ ] **Hızlı yollar:** `PRICE_SINGLE_FACT` ve `FINANCIAL_YOY_SINGLE_COMPANY` deterministik hatlar — "THYAO kaç TL?" bugün 42 saniye sürüyor
- [x] **Sembol sicili (canlıda bulundu, 13 Ağustos 2026):** `extractBistTickers` Türkçe büyük harfli kelimeleri hisse sanıyordu — `GEÇEN YILIN AYNI DÖNEMİYLE` → `['BRSAN','YILIN','AYNI']` → `coklu sirket (3)` → gereksiz sözleşme → tek şirketlik soru 81 saniye. Kara liste sonsuz kümeye karşı çalışmaz; `bist-symbol-registry.cjs` (624 sembol) allowlist olarak otorite. Yan bulgu: sistemde ÜÇÜNCÜ bir sembol dedektörü vardı, tek kaynağa indirildi. Tazeleme: `scripts/refresh-bist-symbol-registry.cjs`
- [x] **Nakit akışı satır eşlemesi (canlıda bulundu, 13 Ağustos 2026):** iki kusur. (a) `^yatırım faaliyetlerinden` üç satıra uyuyor, `rows.find` gelir tablosundaki "Yatırım Faaliyetlerinden Gelirler" (143,9 mn) satırını seçiyordu; gerçek yatırım nakit akışı −3,55 mlr. (b) Finansman satırı İş Yatırım'da "Faaliyetler**den**" yazıyor, desen "Faaliyetlerinden" arıyordu ve satırı hiç yakalamıyordu (−194 mn). Nakit köprüsü tek ayak üstünde kapanıyordu
